...
This project demonstrates a complete ML pipeline from data exploration to model evaluation
...
# Create features based on California housing characteristics
data = {
    'MedInc': np.random.gamma(2, 2, n_samples),  # Median income
    'HouseAge': np.random.uniform(1, 52, n_samples),  # House age
    'AveRooms': np.random.normal(5.4, 2, n_samples),  # Average rooms
    ...
    'AveOccup': np.random.exponential(3, n_samples),  # Average occupancy
...
# Generate target variable (house prices) with realistic correlations
df['MedHouseVal'] = (
    0.4 * df['MedInc'] +
